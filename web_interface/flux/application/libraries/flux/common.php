...
class common
{
            ...
            if (isset($field_name [0]) && $accountinfo ['type'] != 1) {
                if ($field_name [0]->accountid != 0 && $field_name [0]->parent_id == 0) {
                    if ($accountinfo['type'] == 0 || $accountinfo['type'] == 3) {
                        $flag_status = "<a href='../user_list_release/" . $field_name [0]->id . "' title='" . gettext('Release') . "' onClick='return get_reliase_msg();'><span class=' label label-sm label-inverse_blue arrowed_blue-in' title='" . gettext('Release') . "'>" . gettext('Release') . "(C)<span></a>";
                    } else {
                        $flag_status = "<a href='../did_list_release/" . $field_name [0]->id . "' title='" . gettext('Release') . "' onClick='return get_reliase_msg();'><span class=' label label-sm label-inverse_blue arrowed_blue-in' title='" . gettext('Release') . "'>" . gettext('Release') . "(C)<span></a>";
                    }
                    ...
                    $flag_status = "<a href='../did_list_release/" . $field_name [0]->id . "' title='" . gettext('Release') . "' onClick='return get_reliase_msg();'><span class=' label label-sm label-inverse_blue arrowed_blue-in' title='" . gettext('Release') . "'>" . gettext('Release') . "(R)</span></a>";
                } else {
                    $flag_status = "<a href='../did_assgin_reseller/" . $field_name [0]->id . "' title='" . gettext('Assign Number') . "' rel='facebox'><span class=' label label-sm label-inverse_blue arrowed_blue-in' title='" . gettext('Assign Number') . "'>" . gettext('Assign Number') . "</span></a>";
                }
            } else {
    ...
    public function set_invoice_option($select = "", $table = "", $call_type = "", $edit_value = '')
    {
        ...
        if ($select == "" && $this->CI->input->post('sweep_id')) {
            $select = $this->CI->input->post('sweep_id', true);
        }
        if (empty($invoice_date) && $this->CI->input->post('invoice_day')) {
            $invoice_date = $this->CI->input->post('invoice_day', true);
        }
        if ($select == "" || $select == "0") {
            $daily_arr = array(
                    "0" => "0"
    ...
    public function get_invoice_option_selected($select = "", $edit_value = '')
    {
        $uri_segment = $this->CI->uri->segments;
        if (isset($uri_segment [3]) && $uri_segment [3] > 0 && empty($edit_value)) {
            return ($select == "1") ? date('N', strtotime(date('Y-m-d'))) : date('d');
    }
        return ($edit_value > 0) ? $edit_value : (($select == "1") ? date('N', strtotime(date('Y-m-d'))) : date('d'));
    }
    ...
    {
        return ($status == 0) ? gettext("LCR") : gettext("COST");
        ...
        if (! empty($buttons_arr) && $buttons_arr != '') {
            ...
            foreach ($buttons_arr as $button_key => $buttons_params) {
                if ((isset($permissioninfo[$module_name][$sub_module_name][strtolower($button_key)]) && $permissioninfo[$module_name][$sub_module_name][strtolower($button_key)] == 0  or $permissioninfo['login_type'] == '-1' or $permissioninfo['login_type'] == '2' or $permissioninfo['login_type'] == '4' or $permissioninfo['login_type'] == '0' or $permissioninfo['login_type'] == '3')) {
                    ...
                    if (strtoupper($button_key) == "DETRAF") {
                        $ret_url .= $this->build_add_detraf_button($buttons_params->url, $linkid);
                }
            }
        }
    }
    ...
    {
        $link = base_url() . $url . "" . $linkid;
        return '<a href="' . $link . '" class="btn btn-royelblue btn-sm"  title="'.gettext('Download').'" ><i class="fa fa-cloud-download fa-fw"></i></a>&nbsp;';
    }
    public function build_add_detraf_button($url, $linkid)
    {
        $link = base_url() . $url . "" . $linkid;
        return '<a href="' . $link . '" class="btn btn-royelblue btn-sm"  title="'.gettext('Download Report').'" ><i class="fa fa-file-archive-o fa-fw"></i></a>&nbsp;';
    }
    ...
    public function mail_to_users($type, $accountinfo, $attachment = "", $amount = "", $filePath = "")
    {
        ...
        switch ($type) {
            ...
            case 'product_purchase':
                ...
                $message = str_replace('#QUANTITY#', $accountinfo['quantity'], $message);
                $message = str_replace('#TOTAL_PRICE#', isset($accountinfo['total_price']) ? $accountinfo['total_price'] : $accountinfo['price'], $message); 
                $message = str_replace('#NEXT_BILL_DATE#', $accountinfo['next_billing_date'], $message);
                $message = str_replace('#COMPANY_EMAIL#', $settings_reply_email, $message);
                break;
...
$drop_down .=	'<option value="0">' . gettext("--Select--") . '</option>';
	...
	public function set_registration_search_status($select = '')
	{
	    $status_array = array(
	            ""             => gettext("--Select--"),
	            "registered"   => gettext("Registered"),
	            "unregistered" => gettext("Not Registered")
	    );
	    return $status_array;
	}
	public function set_search_bypass_media($select = '')
	{
	    $status_array = array(
	            "" => gettext("--Select--"),
	            "0" => gettext("Active"),

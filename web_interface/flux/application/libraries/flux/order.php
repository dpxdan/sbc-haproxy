    ...
    if ($product_info->product_category == '1') {
		...
		$this->CI->flux_log->write_log('counters_insert_array_log', json_encode($counters_insert_array_log));
		 ...
		 $order_item_array_log = array(
			...
			"next_billing_date" => ($product_info->billing_days == 0)
			    ? gmdate('Y-m-'.$account_info->invoice_day.' 23:59:59', strtotime('+1 month'))
			    : gmdate('Y-m-'.$account_info->invoice_day.' 23:59:59', strtotime('+'.($account_info->invoice_day + $product_info->billing_days).' days')),
			"is_terminated"=>0,
			"termination_date"=>"",

...
class Call_detail_report extends Account
{
      ...
      $this->db->where($object_where_params_date);
		...
		foreach($object_where_params as $object_where_key => $object_where_value) {
			if($object_where_value != '') {	
				...
				if(isset($object_where_key) && $object_where_key == 'disposition'){
					$this->db->where('disposition', $object_where_params['disposition'] );
				}
		...
		if(!empty($where)){
			unset($where['destination'],$where['code'],$where['duration'],$where['disposition'],$where['accountid']);
    ...
    $this->db->select('callstart,sip_user,callerid,call_direction,callednum,notes,billseconds,block_billseconds,disposition,debit,is_recording,country_id,pattern,cost,accountid,reseller_id,pricelist_id,calltype,trunk_id');
    ...
    $cdrs_info = $result->result_array();
        ...
        $cdrs_value['block_duration'] = $cdrs_value['block_billseconds'];
        ...
        unset($cdrs_value['notes'],$cdrs_value['billseconds'],$cdrs_value['block_billseconds'],$cdrs_value['pattern'],$cdrs_value['notes'],$cdrs_value['is_recording']);
			  $cdrsinfo[] =$cdrs_value;
		}
    if (!empty($cdrsinfo)) {
    $this->response ( array (
    ...
    $this->db->select('calltype,callstart,sip_user,call_direction,country_id,callerid,callednum,pattern,notes,billseconds,block_billseconds,provider_call_cost,disposition,provider_id,cost');
    $result = $this->db->get('cdrs');
    $count = $result -> num_rows();
    $cdrs_info = $result->result_array();
		foreach ($cdrs_info as $key => $cdrs_value) { 
            ...
            $cdrs_value['destination'] = $cdrs_value['notes'] ;
            $cdrs_value['code'] =  preg_replace('/[^\d+0-9]/', '',  $cdrs_value['pattern']);
            unset($cdrs_value['notes'],$cdrs_value['billseconds'],$cdrs_value['block_billseconds'],$cdrs_value['pattern'],$cdrs_value['provider_id'],$cdrs_value['is_recording'],$cdrs_value['provider_call_cost']);
			      $cdrsinfo[] =$cdrs_value;
		}
    if (!empty($cdrsinfo)) {
    $this->response ( array (
	...
	private function _reseller_cdrs()
	{
		...
		if($this->postdata['action'] != 'reseller_cdrs_list'){
			 $this->db->select('callstart,callerid,call_direction,callednum,notes,billseconds,block_billseconds,disposition,debit,country_id,pattern,cost,accountid,pricelist_id,calltype,trunk_id');
		}
		else {
			$this->db->where('accountid', $this->postdata['id']);
			$this->db->select('callstart,callerid,callednum,notes,billseconds,block_billseconds,debit,cost,disposition,calltype');
		}

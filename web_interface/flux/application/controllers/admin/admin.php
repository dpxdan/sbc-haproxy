...
/**
 * ****************************************************************
 * IMPORTANT!! : This is API for Admin CURD Operation : IMPORTANT!!
 * ****************************************************************
 *
 * ===================================================
 * API Expected parameters :
 * ===================================================
 * Integer : start_limit (Start limit for admin list)
 * Integer : end_limit (End limit for admin list)
 * Integer : accountid (Unique accountid for each admin)
 * String : first_name (Admin name)
 * Integer : number (Unique account number for each admin)
 * String : email (Unique email id for each admin)
...
class Admin extends Account {
	...
	function __construct() {
		...
		if($this->accountinfo['type'] != -1 && $this->accountinfo ['type'] != 1 && $this->accountinfo ['type'] != 6 && $this->accountinfo ['type'] != 2 ){
			$this->response ( array (
				'status'  => false,
	...
	function _admin_list() {
		if (!isset($this->postdata['end_limit']) || !isset($this->postdata['start_limit'])){
			...
			if($object_where_params['type'] != ''){
				if (!($object_where_params['type'] == '-1' || $object_where_params['type'] == '2' || $object_where_params['type'] == '4'  ) ) {
				$this->response ( array (
					'status' => false,
	...
	function _admin_read() {
		...
		if (empty($postdata['accountid']) || !isset($postdata['accountid'])) {
			...
			$this->response ( array (
				'status'=>true,
				'data' => $read_array,
				'success' => $this->lang->line( "read_admin" ) 
			), 200 );
		}
	}

	function _admin_create() {
			$postdata = $this->postdata;
			...
			if($config != ''){
				$uname = $this->common->find_uniq_rendno_admin(common_model::$global_config['system_config']['card_length'], 'number ', 'accounts');	
			}else{
				$uname = $this->common->find_uniq_rendno_admin(10, 'number ', 'accounts');
			}
	        ...
	        if ($pin_generate == 0 ) {
		        ...
		        $pin_number = $this->common->find_uniq_rendno_admin($pin, 'number', 'accounts');
		    }
		...
		if(!empty($accountinfo)) {
			if($postdata['action'] != 'provider_create'){
				$this->response ( array (
					'status'=> true,
					'data' => $accountinfo,
					'success' => $this->lang->line('create_admin_success') 
				), 200 );
			}else{
				$this->response ( array (
					'status'=> true,

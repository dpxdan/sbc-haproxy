...
<head>
	...
	<title>
		...
		$user_header_logo = $this->session->userdata('user_logo');
		$user_icon = $this->session->userdata('user_favicon');
	...
	<?php } else {  ?>
		<link rel="icon" href="<? echo base_url(); ?>upload/<? echo $user_icon ?>" />
	<?php } ?>
	<script>
		...
		var procmsg_gettext = "<?php echo gettext('Processing, please wait ...'); ?>";
		var date = "<?php echo gettext($date); ?>";

		var gettext_custom_cache = <?php
								echo json_encode(array(
									'Page'                         => gettext('Page'),
									'of'                           => gettext('of'),
									'Records'                      => gettext('Records'),
									'No Records'                   => gettext('No Records'),
									'Processing, please wait ...'   => gettext('Processing, please wait ...'),
								), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG);
								?>;

		var button_permission_cache = <?php
										$perm_info    = $this->session->userdata('permissioninfo');
										$perm_module  = $this->uri->segment(1);
										$perm_submod  = $this->uri->segment(2);
										$perm_map     = array();

										$perm_login_type = is_array($perm_info) && isset($perm_info['login_type'])
											? $perm_info['login_type']
											: null;
										$perm_bypass = ($perm_login_type == '-1' || $perm_login_type == '0' || $perm_login_type == '3');

										if (is_array($perm_info) && $perm_module !== '' && $perm_submod !== '') {
											if (isset($perm_info[$perm_module][$perm_submod]) && is_array($perm_info[$perm_module][$perm_submod])) {
												foreach ($perm_info[$perm_module][$perm_submod] as $perm_button => $perm_value) {
													$perm_map[$perm_button] = ($perm_bypass || $perm_value == 0) ? '0' : '1';
												}
											}
										}
										echo json_encode($perm_map, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG);
										?>;

		var button_permission_allow_all = <?php echo $perm_bypass ? 'true' : 'false'; ?>;
	</script>
...
<body>
	...
	if (empty($user_logo['logo'])) {
		$user_logo['logo'] = $user_header_logo;
	}
	?>
	<nav class="navbar navbar-expand-lg navbar-light bg-light mainmenu px-lg-4">
		...
		<?php if ($this->session->userdata('userlevel_logintype') != '0') { ?>
			<a class="navbar-brand p-lg-0 m-0" href="<?php echo base_url(); ?>dashboard/">
				<img id="logo" class="img-fluid" alt="dashboard" src="/upload/<?php echo $user_logo['logo']; ?>">
			</a>
		<?php } else { ?>
			<a class="navbar-brand p-lg-0 m-0" href="<?php echo base_url(); ?>user/user/">
				<img id="logo" class="img-fluid" alt="user_logo" src="/upload/<?php echo $user_logo['logo']; ?>">
			</a>
		<?php } ?>
			...
			<div class="col navbar-profile dropdown">
				...
				if ($this->session->userdata('userlevel_logintype') != '0') { ?>
					...
					<? } else { ?>
						...
						<? }
					if ($this->session->userdata('logintype') != 2 && $this->session->userdata('logintype') != -1 && $this->session->userdata('logintype') != 4 && $this->session->userdata('logintype') != 0) {

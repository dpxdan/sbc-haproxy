...
<footer class="site-footer border_box">
  ...
  <?php
 /*
$footer = $this->session->userdata['user_footer'];

  if (!isset($footer) || $footer == '') {
    $this->db->select('website_footer');
    $this->db->where('id', 1);
  ...
  $userlevel_logintype = $this->session->userdata['userlevel_logintype'];
    if($this->session->userdata['logintype'] == 1 || $this->session->userdata['logintype'] == 0 ){
     $user_footer = $this->session->userdata('user_footer');
     if(isset($user_footer) && $user_footer != '') {
         ?>
           <div class="col-md-8 py-2 px-2 text-md-left text-light"><?php echo $user_footer?></div>
           <?php }else{?>
            <div class="col-md-8 py-2 px-2 text-md-left text-light">Copyright @ <?php echo date("Y"); ?> <a class="text-warning" href="http://flux.net.br" target="_blank"> <?php echo gettext("Flux Telecom")?></a>. <?php echo gettext("All Rights Reserved.")?>
                </div>
      <?php }?>
	...
	<?php	} else  if($this->session->userdata['logintype'] ==0 || $this->session->userdata['logintype'] == 1){
		...
		if ($user_footer == '') {
			...
			?>
           <div class="col-md-8 py-2 px-2 text-md-left text-light">Copyright @ <?php echo date("Y"); ?> <a class="text-warning" href="http://flux.net.br" target="_blank"> <?php echo gettext("Flux Tecnologia.")?></a>. <?php echo gettext("All Rights Reserved.")?>
  ...
  */
    ?>
    <?php $user_footer = $this->session->userdata('user_footer'); ?>

<?php if (!empty($user_footer)) : ?>
    <div class="col-md-8 py-2 px-2 text-md-left text-light">
        <?= $user_footer ?>
    </div>
<?php else : ?>
    <div class="col-md-8 py-2 px-2 text-md-left text-light">
        Copyright @ <?= date('Y') ?>
        <a class="text-warning" href="http://flux.net.br" target="_blank"><?= gettext('Flux Telecom') ?></a>.

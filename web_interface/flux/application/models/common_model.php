...
class Common_model extends CI_Model {
	...
	function calculate_currency($amount = 0, $from_currency = '', $to_currency = '', $format_currency = true, $append_currency = true) {
		...
		$to_currency = ($from_currency == '') ? self::$global_config ['system_config'] ['base_currency'] : $from_currency;
		//		$currency_id_currency = $this->common->get_field_name ( 'currency', 'currency', $from_currency );
		//		$currency_id = $this->common->get_field_name('id','currency' , array("currency" => $from_currency));
		/*if ($to_currency == '') {
	...
	function get_dashboard_details(){
		...
		if(isset($data_array['no_of_calls']['billseconds'])){
	...
	function get_list_domains() {
		$this->db->select ( 'id,domain,domain_description' );

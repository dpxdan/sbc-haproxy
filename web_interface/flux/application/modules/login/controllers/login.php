...
class Login extends MX_Controller
{
    ...
    public function set_lang_global($post = false)
    {
...
//                        $logintype = $result['type'] == -1 ? 2 : $result['type'];
                        ...
                        $logo_arr = $res->result();
                        if ($logo_arr[0]->reseller_id == 0 && $result['type'] == 1 && ($logo_arr[0]->website_title == "" || $logo_arr[0]->website_footer == ""))
                        {
                            $invoice_admin_result = $this->db_model->getSelect("*", "invoice_conf", array(
                                'id' => 1
                            ));
                            $invoice_admin_result = $invoice_admin_result->result_array();
                            $invoice_admin_result = $invoice_admin_result[0];
                            $this->flux_log->write_log('invoice_admin_result', json_encode($invoice_admin_result));

                            $this->flux_log->write_log('invoice_user_result', json_encode($logo_arr[0]));

                            $logo_arr[0]->website_title = $invoice_admin_result['website_title'];
                            $logo_arr[0]->website_footer = $invoice_admin_result['website_footer'];

                            $admin_website_title = $invoice_admin_result['website_title'];
                            $admin_website_footer = $invoice_admin_result['website_footer'];
                        }
                        $data['user_logo'] = (isset($logo_arr[0]->logo) && $logo_arr[0]->logo != "") ? $logo_arr[0]->accountid . "_" . $logo_arr[0]->logo : "logo.png";
                        $data['user_header'] = (isset($logo_arr[0]->website_title) && $logo_arr[0]->website_title != "") ? $logo_arr[0]->website_title : $admin_website_title;
                        $data['user_footer'] = (isset($logo_arr[0]->website_footer) && $logo_arr[0]->website_footer != "") ? $logo_arr[0]->website_footer : $admin_website_footer;
            ...
            if (isset($_SERVER['HTTP_HOST']))
            {
                ...
                if ($logo_arr[0]->reseller_id == 0 && $result['type'] == 1 && ($logo_arr[0]->website_title == "" || $logo_arr[0]->website_footer == ""))
                {
                    ...
                    $invoice_admin_result = $invoice_admin_result->result_array();
                    $invoice_admin_result = $invoice_admin_result[0];
                    $this->flux_log->write_log('invoice_admin_result2', json_encode($invoice_admin_result));

                    $this->flux_log->write_log('invoice_user_result2', json_encode($logo_arr[0]));
                ...
                $data['website_header'] = (isset($logo_arr[0]->website_title) && $logo_arr[0]->website_title != "") ? $logo_arr[0]->website_title : $admin_website_title;
                $data['website_footer'] = (isset($logo_arr[0]->website_footer) && $logo_arr[0]->website_footer != "") ? $logo_arr[0]->website_footer : $admin_website_footer;
    ...
    public function index()
    {
        ...
        if ($this->session->userdata('user_login') == false)
        {
            if (! empty($_POST) && isset($_POST['username']) && isset($_POST['password']) && trim($_POST['username']) != '' && trim($_POST['password']) != '')
            {
                ...
                if ($user_valid == 1)
                {
                    $this->session->set_userdata('user_login', true);
                    $where  = "(number = '" . $this->db->escape_str($_POST['username']) . "' OR email = '" . $this->db->escape_str($_POST['username']) . "') and deleted = 0";
                    $result = $this->db_model->getSelect("*", "accounts", $where)->result_array();
                    $result = $result[0];
                    ...
                    if ($user_multi_level == 0)
                    {
                        ...
                        $this->session->set_userdata('username', $_POST['username']);
                        $this->session->set_userdata('accountinfo', $result);
                        $permission_result  = $this->db_model->getSelect("*", "permissions", array('id' => $result['permission_id']))->result_array();
                        $permission_result  = $permission_result[0];
                        $permission_decode  = json_decode($permission_result['permissions'], true);
                        $permission_decode['login_type'] = $result['type'];
                        $this->session->set_userdata('permissioninfo', $permission_decode);
                        ...
                        {
                            $this->db->where(array(
                                "accountid" => $result['reseller_id'] != 0 ? $result['reseller_id'] : "1"
                            ));
                        }
                        elseif ($result['type'] == '1')
                        {
                            if ($result['reseller_id'] == 0)
                            {
                                ...
                                $this->db->where(array("accountid" => $result_invoice ? $result['id'] : "1"));
                            }
                            else
                            {
                                $result_invoice = $this->common->get_field_name('id', 'invoice_conf', array("accountid" => $result['reseller_id']));
                                $this->db->where(array("accountid" => $result_invoice ? $result['reseller_id'] : "1"));
                            }
                        }
                        else
                        {
                        ...
                        $logo_arr = $this->db->get("invoice_conf")->result();

                        // ── fallback para type 1 sem branding próprio ──────────────────
                        $admin_website_title  = '';
                        $admin_website_footer = '';
                        if ($result['type'] == 1 && (empty($logo_arr[0]->website_title) || empty($logo_arr[0]->website_footer)))
                        {
                            $invoice_admin_result = $this->db_model->getSelect('*', 'invoice_conf', array('id' => 1))->row_array();
                            if (! empty($invoice_admin_result))
                            {
                                $logo_arr[0]->website_title  = $logo_arr[0]->website_title  ?: $invoice_admin_result['website_title'];
                                $logo_arr[0]->website_footer = $logo_arr[0]->website_footer ?: $invoice_admin_result['website_footer'];
                                $admin_website_title         = $invoice_admin_result['website_title'];
                                $admin_website_footer        = $invoice_admin_result['website_footer'];
                            }
                        }

                        // ── grava branding na sessão ───────────────────────────────────
                        $this->session->set_userdata(array(
                            'user_logo'    => ! empty($logo_arr[0]->logo)
                                              ? $logo_arr[0]->accountid . '_' . $logo_arr[0]->logo
                                              : 'logo.png',
                            'user_header'  => ! empty($logo_arr[0]->website_title)
                                              ? $logo_arr[0]->website_title
                                              : $admin_website_title,
                            'user_footer'  => ! empty($logo_arr[0]->website_footer)
                                              ? $logo_arr[0]->website_footer
                                              : $admin_website_footer,
                            'user_favicon' => ! empty($logo_arr[0]->favicon)
                                              ? $logo_arr[0]->accountid . '_' . $logo_arr[0]->favicon
                                              : 'favicon.ico',
                        ));
            ...
            if (isset($_SERVER['HTTP_HOST']))
            {
                ...
                // ── fallback para type 1 (só quando login foi tentado) ─────────
                $admin_website_title  = '';
                $admin_website_footer = '';
                if (isset($result) && $result['type'] == 1 && (empty($logo_arr[0]->website_title) || empty($logo_arr[0]->website_footer)))
                {
                    $invoice_admin_result = $this->db_model->getSelect('*', 'invoice_conf', array('id' => 1))->row_array();
                    if (! empty($invoice_admin_result))
                    {
                ...
                $data['user_logo']      = ! empty($logo_arr[0]->logo)
                                          ? $logo_arr[0]->logo
                                          : 'logo.png';
                $data['website_header'] = ! empty($logo_arr[0]->website_title)
                                          ? $logo_arr[0]->website_title
                                          : $admin_website_title;
                $data['website_footer'] = ! empty($logo_arr[0]->website_footer)
                                          ? $logo_arr[0]->website_footer
                                          : $admin_website_footer;
                $data['user_favicon']   = ! empty($logo_arr[0]->favicon)
                                          ? $logo_arr[0]->accountid . '_' . $logo_arr[0]->favicon
                                          : 'favicon.ico';

                $this->session->set_userdata(array(
                    'user_logo'    => $data['user_logo'],
                    'user_header'  => $data['website_header'],
                    'user_footer'  => $data['website_footer'],
                    'user_favicon' => $data['user_favicon'],
                ));
            }
        ...
        else
        {
            ...
            $custom_domain = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on")
                             ? "https://" . $_SERVER["HTTP_HOST"]
                             : "http://" . $_SERVER["HTTP_HOST"];
            ...
            $this->session->set_userdata(array(
                ...
                'user_footer'  => ! empty($logo_arr[0]->website_footer)
                                  ? $logo_arr[0]->website_footer
                                  : 'Flux Telecom All Rights Reserved.',
                'user_favicon' => ! empty($logo_arr[0]->favicon)
                                  ? $logo_arr[0]->accountid . '_' . $logo_arr[0]->favicon
                                  : 'favicon.ico',
            ));
    ...
    public function get_language_text()
    {
    ...
    public function encrypt($api_key, $id = false)
    {
        $str = $this->encode($api_key . "#" . $id . "_" . $api_key);
        return $this->encode_params($str);
    }

    public function login_as_reseller($select_id)
    {
    ...
    public function login_as_customer($select_id)
    {
        $accountinfo = $this->session->userdata('accountinfo');
        $where = array(
            'id' => $select_id
    ...
    public function login_as_admin($select_id)
    {
        $this->session->sess_destroy();
        redirect(base_url() . "relogin/" . $select_id . "/0/");
    }

    public function relogin($new_login_id, $master_id = '0')
    {
        $where = array(
            'id' => $new_login_id
        );
    ...
    public function customer_permission_list()
    {

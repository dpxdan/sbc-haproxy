...
class Accounts extends MX_Controller {
	...
	function provider_edit_block_pattern($accountid, $patternid) {
		$this->customer_edit_block_pattern($accountid, $patternid);
	}

	function customer_edit_block_pattern($accountid, $patternid) {
		...
		$pattern_res = $this->db_model->getSelect("id,blocked_patterns,destination,direction", "block_patterns", $where);
		if ($pattern_res->num_rows() > 0) {
			$pattern_data          = (array) $pattern_res->first_row();
			$data['page_title']    = gettext('Edit Block Code');
			$data['accountid']     = $accountid;
			$data['patternid']     = $patternid;
			$data['code']          = $this->common->get_only_numeric_val('', '', $pattern_data['blocked_patterns']);
			$data['destination']   = $pattern_data['destination'];
			$data['direction']     = $pattern_data['direction'];
			$this->load->view('view_block_manual_prefix_edit', $data);
		} else {
			$this->session->set_flashdata('flux_notification', gettext('Permission Denied!'));
			redirect(base_url().'accounts/customer_list/');
			exit();
	...
	function provider_update_block_prefix($accountid, $patternid) {
		$this->customer_update_block_prefix($accountid, $patternid);
	}

	function customer_update_block_prefix($accountid, $patternid) {
		$prefix    = preg_replace('/[^0-9]/', '', $this->input->post('prefix', true));
		$direction = $this->input->post('direction', true) ?: 'outbound';
		...
		$result = $this->accounts_model->update_manual_block(
			$patternid,
			$prefix,
			$this->input->post('destination', true),
			$direction,
	...
	function customer_delete_block_pattern($accountid, $patternid) {
		...
		$this->permission->check_web_record_permission($patternid, 'block_patterns', 'accounts/customer_list/', true, array(
				'field_name'   => "accountid",
				"parent_table" => "accounts",
			));
		$accountinfo = $this->session->userdata('accountinfo');
		...
		if (!empty($entity_type)) {
			$entity_type = strtolower($this->common->get_entity_type('', '', $entity_type));
			$url         = "accounts/customer_blocked_prefixes/$accountid/";
	...
	function customer_invoice_option($value = false) {
		...
		$drp_array = $this->common->set_invoice_option($sweepid, "", "", $value);
		$selected  = $this->common->get_invoice_option_selected($sweepid, $value);
		if ($sweepid == "1") {
			$invoice_dropdown = form_dropdown(array(
					"name"  => 'invoice_day',

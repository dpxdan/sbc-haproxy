...
class Fsmonitor extends CI_Controller {
    ...
    function sip_devices_json($id=0){
        $json_data = array();
        $json_data['total'] = '';
        $profiles = $this->fsmonitor_model->get_active_sip_profiles();
        foreach ($profiles as $profile) {
            $command = "api sofia xmlstatus profile " . $profile['name'] . " reg";
        ...
        if (isset($json_data['rows'])) {
            $json_data['total'] = count($json_data['rows']);
        } else {
            $json_data['row'] = '';
        }
        echo json_encode($json_data);
    ...
    function sip_devices() {
		...
		$query = $this->db_model->getSelect("*", "freeswich_servers", "");
	    $data['fs_data']    = $query->result_array();
		$this->flux_log->write_log('account_data', json_encode($account_data));
                    ...
                    if (array_key_exists("registration", $response_arr["registrations"])) {
                        if (!array_key_exists("0", $response_arr["registrations"])) {
                            ...
                            if (array_key_exists("0", $response_final_arr["registration"])) {
                                ...
                                $account_id = $this->common->get_field_name('accountid', 'sip_devices', array('username' => @$response_final_arr['registration']['sip-auth-user']));
                                $acc_id = ($account_id == 0) ? 1 : $account_id;
                                if ($acc_id == $account_info['id']) {
                                    foreach ($response_final_arr as $response_value) {
                                        $json_data['rows'][] = array('cell' => array(

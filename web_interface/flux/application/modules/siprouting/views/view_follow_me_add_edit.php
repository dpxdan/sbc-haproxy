...
<script>
	...
	function validateForm() {
		var err = '';
		if (document.forms["follow_me_form"]["call_forwarding_flag"].value == 0
			&& document.forms["follow_me_form"]["call_forwarding_destination"].value == "") {
		...
		if (document.forms["follow_me_form"]["on_busy_flag"].value == 0
			&& document.forms["follow_me_form"]["on_busy_destination"].value == "") {
		...
		if (document.forms["follow_me_form"]["no_answer_flag"].value == 0
			&& document.forms["follow_me_form"]["no_answer_destination"].value == "") {
		...
		if (document.forms["follow_me_form"]["not_register_flag"].value == 0
			&& document.forms["follow_me_form"]["not_register_destination"].value == "") {
...
<script>
	function change_extension_call_forward(exten_num) {
		exten_num = (typeof exten_num !== 'undefined' && exten_num !== '') ? exten_num : '';
		...
		$.ajax({
			...
			data: {
				name           : "call_forwarding_destination",
				extension_value: extension_call_forward,
				accountid      : accountid,
				exten_num      : exten_num
			},
			success: function(response) {
				$("#call_forwarding_destination_container").html(response);
	...
	function change_extension_on_busy(exten_num) {
		...
		$.ajax({
			...
			url: "<?php echo base_url(); ?>siprouting/fssipdevices_build_extension_dropdown",
			data: {
				name           : "on_busy_destination",
				extension_value: extension_on_busy,
				accountid      : accountid,
				exten_num      : exten_num
			},
			success: function(response) {
				$("#on_busy_destination_container").html(response);
				$('.selectpicker').selectpicker('refresh');
			}
		});
	}
	...
	function change_extension_no_answer(exten_num) {
		exten_num = (typeof exten_num !== 'undefined' && exten_num !== '') ? exten_num : '';
		var extension_no_answer = $("#extension_no_answer").val();
		var accountid = $("#accountid").val();
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>siprouting/fssipdevices_build_extension_dropdown",
			data: {
				...
				extension_value: extension_no_answer,
				accountid      : accountid,
				exten_num      : exten_num
			},
			success: function(response) {
				$("#no_answer_destination_container").html(response);
				$('.selectpicker').selectpicker('refresh');
			}
		});
	}
	...
	function change_extension_not_registered(exten_num) {
		exten_num = (typeof exten_num !== 'undefined' && exten_num !== '') ? exten_num : '';
		var extension_not_registered = $("#extension_not_registered").val();
		var accountid = $("#accountid").val();
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>siprouting/fssipdevices_build_extension_dropdown",
			data: {
				name           : "not_register_destination",
				extension_value: extension_not_registered,
				accountid      : accountid,
				exten_num      : exten_num
			},
			success: function(response) {
				$("#not_register_destination_container").html(response);
				$('.selectpicker').selectpicker('refresh');
			}
		});
	}
												...
												<div class="card col-md-12 p-0">
													...
													<div class="alert-white align-items-center p-3 col-md-12 d-flex justify-content-between">
														...
														<div class="text-right">
															<label class="switch">
																<input type="checkbox" name="call_forwarding_flag" id="call_forwarding_flag_enabled" class="onoffswitch-checkbox" onclick="call_forwarding_enabled_disabled();" value="0" <?php if ($edit_array['call_forwarding_flag'] == 0) {
													...
													<div class="col-md-12" id="extension_call_forward_disabled">
														<div class="row p-4">
															<div class="col-md-12 mb-2">
																<label><?php echo gettext("Tipo"); ?></label>
																<select name="extension_call_forward" id="extension_call_forward" onchange="change_extension_call_forward()" class="form-control form-control-lg col-md-12 float-left selectpicker" data-live-search="true">
																	<option value="2" <?php if ($edit_array['extension_call_forward'] == '2') {
																	    echo "selected=selected";
																	} ?>><?php echo gettext("Ramal"); ?></option>
																	<option value="3" <?php if ($edit_array['extension_call_forward'] == '3') {
																	    echo "selected=selected";
																	} ?>><?php echo gettext("DID"); ?></option>
																	<option value="1" <?php if ($edit_array['extension_call_forward'] == '1') {
																	    echo "selected=selected";
																	} ?>><?php echo gettext("PSTN"); ?></option>
																</select>
															</div>
															<div class="col-md-12" id="call_forwarding_destination_container"></div>
														</div>
													</div>
												...
												<div class="card col-md-12 p-0">
													<div class="">
														<div class="alert-white align-items-center p-3 col-md-12 d-flex justify-content-between">
															...
															<div>
																<label class="switch">
																	<input type="checkbox" name="on_busy_flag" class="onoffswitch-checkbox" onclick="on_busy_enabled_disabled()" value="0" <?php if ($edit_array['on_busy_flag'] == 0) {
																	    echo "checked=checked";
																	} ?>>
																	<span class="slider round"></span>
																</label>
														...
														<div class="col-md-12" id="on_busy_disabled">
															<div class="row p-4">
																<div class="col-md-12 mb-2">
																	<label><?php echo gettext("Tipo"); ?></label>
																	<select name="extension_on_busy" id="extension_on_busy" onchange="change_extension_on_busy()" class="form-control form-control-lg col-md-12 float-left selectpicker" data-live-search="true">
																		<option value="2" <?php if ($edit_array['extension_on_busy'] == '2') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("Ramal"); ?></option>
																		<option value="3" <?php if ($edit_array['extension_on_busy'] == '3') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("DID"); ?></option>
																		<option value="1" <?php if ($edit_array['extension_on_busy'] == '1') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("PSTN"); ?></option>
																	</select>
																</div>
																<div class="col-md-12" id="on_busy_destination_container"></div>
															</div>
														</div>
												...
												<div class="card col-md-12 p-0">
													<div class="">
														<div class="alert-white align-items-center p-3 col-md-12 d-flex justify-content-between">
															...
															<div>
																<label class="switch">
																	<input type="checkbox" name="no_answer_flag" onclick="no_answer_disabled_enabled()" class="onoffswitch-checkbox" value="0" <?php if ($edit_array['no_answer_flag'] == 0) {
																	    echo "checked=checked";
																	} ?>>
																	<span class="slider round"></span>
																</label>
														...
														<div class="col-md-12" id="no_answer_disabled">
															<div class="row p-4">
																<div class="col-md-12 mb-2">
																	<label><?php echo gettext("Tipo"); ?></label>
																	<select name="extension_no_answer" id="extension_no_answer" onchange="change_extension_no_answer()" class="form-control form-control-lg col-md-12 float-left selectpicker" data-live-search="true">
																		<option value="2" <?php if ($edit_array['extension_no_answer'] == '2') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("Ramal"); ?></option>
																		<option value="3" <?php if ($edit_array['extension_no_answer'] == '3') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("DID"); ?></option>
																		<option value="1" <?php if ($edit_array['extension_no_answer'] == '1') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("PSTN"); ?></option>
																	</select>
																</div>
																<div class="col-md-12" id="no_answer_destination_container"></div>
															</div>
														</div>
												...
												<div class="card col-md-12 p-0">
													<div class="">
														<div class="alert-white align-items-center p-3 col-md-12 d-flex justify-content-between">
															...
															<div>
																<label class="switch">
																	<input type="checkbox" name="not_register_flag" onclick="not_register_disabled_enabled()" class="onoffswitch-checkbox" value="0" <?php if ($edit_array['not_register_flag'] == 0) {
																	    echo "checked=checked";
																	} ?>>
																	<span class="slider round"></span>
																</label>
														...
														<div class="col-md-12" id="not_register_disabled">
															<div class="row p-4">
																<div class="col-md-12 mb-2">
																	<label><?php echo gettext("Tipo"); ?></label>
																	<select name="extension_not_registered" id="extension_not_registered" onchange="change_extension_not_registered()" class="form-control form-control-lg col-md-12 float-left selectpicker" data-live-search="true">
																		<option value="2" <?php if ($edit_array['extension_not_registered'] == '2') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("Ramal"); ?></option>
																		<option value="3" <?php if ($edit_array['extension_not_registered'] == '3') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("DID"); ?></option>
																		<option value="1" <?php if ($edit_array['extension_not_registered'] == '1') {
																		    echo "selected=selected";
																		} ?>><?php echo gettext("PSTN"); ?></option>
																	</select>
																</div>
																<div class="col-md-12" id="not_register_destination_container"></div>
															</div>
														</div>
										...
										<div class="col-md-6 p-0 pr-lg-1">
											<div class="card p-3">
												...
												<select name="is_recording" id="is_recording" class="dropdown bootstrap-select form-control form-control-lg col-md-12 float-left selectpicker" data-live-search="true" tabindex="-98">
													<option value="1" <?php if ($edit_array['is_recording'] == '1') {
													    echo "selected=selected";
													} ?>><?php echo gettext("No"); ?></option>
													<option value="0" <?php if ($edit_array['is_recording'] == '0') {
													    echo "selected=selected";

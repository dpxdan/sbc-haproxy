...
class Siprouting extends MX_Controller
{
    ...
    public function fssipdevices_routing_save1()
    {
        ...
        if ($add_array['id'] == '') {
            ...
            $this->session->set_flashdata('flux_errormsg', gettext('Sip Routing added successfully!'));
        } else {
            $query = $this->siprouting_model->edit_sip_device_routing($add_array, $add_array['id']);
            $this->session->set_flashdata('flux_errormsg', gettext('Sip Routing updated successfully!'));
        }
	...
	public function fssipdevices_routing_save()
{
    $add_array = $this->input->post();

    if (!isset($add_array['call_forwarding_flag']))  $add_array['call_forwarding_flag']  = 1;
    if (!isset($add_array['on_busy_flag']))          $add_array['on_busy_flag']          = 1;
    if (!isset($add_array['no_answer_flag']))        $add_array['no_answer_flag']        = 1;
    if (!isset($add_array['not_register_flag']))     $add_array['not_register_flag']     = 1;
    if (!isset($add_array['follow_me_flag']))        $add_array['follow_me_flag']        = 1;
    if (!isset($add_array['ingnore_busy_flag']))     $add_array['ingnore_busy_flag']     = 1;
    if (!isset($add_array['do_not_disturb']))        $add_array['do_not_disturb']        = 1;

    $redirect_url = $add_array['redirect_url'];
    unset($add_array['redirect_url']);

    // Mapear extension_* (POST) para colunas de tipo
    $add_array['call_forwarding_destination_type']  = isset($add_array['extension_call_forward'])    ? (int) $add_array['extension_call_forward']    : 2;
    $add_array['on_busy_destination_type']          = isset($add_array['extension_on_busy'])         ? (int) $add_array['extension_on_busy']         : 2;
    $add_array['no_answer_destination_type']        = isset($add_array['extension_no_answer'])       ? (int) $add_array['extension_no_answer']       : 2;
    $add_array['not_register_destination_type']     = isset($add_array['extension_not_registered'])  ? (int) $add_array['extension_not_registered']  : 2;
    ...
    if (!isset($add_array['pickup_ids']))       $add_array['pickup_ids']       = array();
    if (!isset($add_array['allow_pickup_ids'])) $add_array['allow_pickup_ids'] = array();

    if ($add_array['id'] == '') {
        $this->siprouting_model->add_sip_device_routing($add_array);
        $this->session->set_flashdata('flux_errormsg', gettext('Sip Routing added successfully!'));
    } else {
        $this->siprouting_model->edit_sip_device_routing($add_array, $add_array['id']);
        $this->session->set_flashdata('flux_errormsg', gettext('Sip Routing updated successfully!'));
    }
    ...
    public function fssipdevices_voicemail($edit_id = '', $type = "")
    {
    ...
    public function fssipdevices_voice_mail_save($edit_id = '')
    {
        $add_array = $this->input->post();
        $redirect_url = $add_array['redirect_url'];
        unset($add_array['redirect_url']);
    ...
    public function fssipdevices_voicemail_file_play($file_name)
    {
    ...
    public function fssipdevices_routing($id = '')
    {
        ...
        if (isset($id) && $id != "") {
            ...
            if ($sip_device_routing_details->num_rows() > 0) {
				...
				*/
				...
				$sip_routing_data['extension_call_forward']   = isset($sip_routing_data['call_forwarding_destination_type'])  ? $sip_routing_data['call_forwarding_destination_type']  : 2;
				$sip_routing_data['extension_on_busy']        = isset($sip_routing_data['on_busy_destination_type'])          ? $sip_routing_data['on_busy_destination_type']          : 2;
				$sip_routing_data['extension_no_answer']      = isset($sip_routing_data['no_answer_destination_type'])        ? $sip_routing_data['no_answer_destination_type']        : 2;
				$sip_routing_data['extension_not_registered'] = isset($sip_routing_data['not_register_destination_type'])     ? $sip_routing_data['not_register_destination_type']     : 2;
    ...
    public function fssipdevices_build_extension_dropdown1()
    {
    ...
    public function fssipdevices_build_extension_dropdown()
    {
        $extension_value = $_POST['extension_value'];
        $name            = $_POST['name'];
        $accountid       = isset($_POST['accountid']) ? (int) $_POST['accountid'] : 0;
        $value           = (isset($_POST['exten_num']) && $_POST['exten_num'] != '')
                           ? str_replace('#', '', $_POST['exten_num'])
                           : '';

        if ($extension_value == '2') {
            // Ramal: dropdown de SIP devices da conta
            $output = $this->common->sip_dropdown($name, $accountid, $value);

        } elseif ($extension_value == '3') {
            // DID: dropdown de DIDs da própria conta
            $this->db->select('number');
            $this->db->where('accountid', $accountid);
            $this->db->order_by('number', 'ASC');
            $query = $this->db->get('dids');

            $options = array('' => ''.gettext("--Select--").'');
            if ($query->num_rows() > 0) {
                foreach ($query->result_array() as $row) {
                    $options[$row['number']] = $row['number'];
                }
            }
            ...
            $dropdown_params = array(
                'name'             => $name,
                'id'               => $name,
                'class'            => 'form-control form-control-lg col-md-12 float-left selectpicker ' . $name,
                'data-live-search' => 'true',
            );
            $output = form_dropdown($dropdown_params, $options, $value);

        } else {
            // PSTN: input de texto livre
            $output = '<input type="text"'
                . ' value="' . htmlspecialchars($value, ENT_QUOTES) . '"'
                . ' name="' . $name . '"'
                . ' id="' . $name . '"'
                . ' class="form-control form-control-lg col-md-12 float-left ' . $name . '">';
        }

        echo $output;
    }
    public function fssipdevices_get_music_on_hold_dropdown()
    {
        $add_array = $this->input->post();

...
class Freeswitch extends MX_Controller
{
    ...
    function fssipdevices_clearsearchfilter()
    {
        ...
        $this->session->set_userdata('fssipdevices_reg_filter', "");
    }
    ...
    function fssipdevices_search()
    {
        ...
        if ($this->input->post('advance_search', TRUE) == 1) {
            ...
            unset($action['action']);
            unset($action['advance_search']);
            // Filtro de registro nao e coluna do banco: isolar em session propria
            // para nao entrar no build_search()
            $reg_filter = isset($action['registration_status']) ? $action['registration_status'] : '';
            unset($action['registration_status']);
            $this->session->set_userdata('fssipdevices_reg_filter', $reg_filter);
    ...
    function fssipdevices_json()
    {
        ...
        $new_array = $this->freeswitch_model->get_registered_usernames();
        $reg_filter = $this->session->userdata('fssipdevices_reg_filter');
        $advance = $this->session->userdata('advance_search');
        if ($advance == 1 && $reg_filter != '') {
            $all = $this->freeswitch_model->fs_retrieve_sip_user(true, 0, 0);
            $filtered = array();
            foreach ($all as $device) {
                $is_registered = in_array($device['username'], $new_array);
                if (($reg_filter == 'registered' && $is_registered) || ($reg_filter == 'unregistered' && !$is_registered)) {
                    $filtered[] = $device;
                }
            }
            $count_all = count($filtered);
            $paging_data = $this->form->load_grid_config($count_all, $_GET['rp'], $_GET['page']);
            $json_data = $paging_data["json_paging"];
            $query = array_slice($filtered, $paging_data["paging"]["start"], $paging_data["paging"]["page_no"]);
        } else {

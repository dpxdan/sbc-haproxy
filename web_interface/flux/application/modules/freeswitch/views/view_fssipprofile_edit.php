...
$(document).ready(function() {
  ...
  jQuery('#sip_port').removeClass('borderred');
   ...
   function validateForm(){
      ...
      if(formflag){
      	alert("<?php echo gettext('Request to reload the sip profile to reflect the applied changes in Flux SBC.'); ?>");
		$('#myForm1').submit();
	  }
}
...
function reload_port(h){
	var sip_port = confirm("<?php echo gettext('Are you sure about the applied config changed? This will affect how the calls will be accepted and handled by Flux SBC. Please note: If the switch is currently having live calls, the reload will not reflect the applied changes in Flux SBC.'); ?>");
		if(sip_port == true){

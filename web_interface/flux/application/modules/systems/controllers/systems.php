    ...
    function database_backup_save()
    {
        ...
        if ($add_array['id'] != '') {} else {
            if ($this->form_validation->run() == FALSE) {
                ...
                $run_backup = "/usr/bin/mysqldump --single-transaction --databases " . $db_name . " -h'" . $db_hostname . "' --protocol=TCP -u'" . $db_username . "' -p'" . $db_password . "' > '$backup_file'";
                $error_zip = 0;
    ...
    function database_restore_one($id = '')
    {
        ...
        if ($result->num_rows() > 0) {
            ...
            if (file_exists($path)) {
                ...
                $MYSQL = "/usr/bin/mysql";
                $run_restore = $run_gzip . $MYSQL . " -h'" . $db_hostname . "' --protocol=TCP -u'" . $db_username . "' -p'" . $db_password . "' " . $db_name;
                exec($run_restore);

...
,reseller_products.product_id', array(
	...
	{
        if (!isset($this->session->userdata['userlevel_logintype']))
        {
        $this->session->userdata['userlevel_logintype'] = $accountinfo['type'];
        }
		if ($this->session->userdata['userlevel_logintype'] == '-1' || $this->session->userdata['userlevel_logintype'] == '2' || $accountinfo['type'] == '-1' )
		{
	...
	function update_did_forward($data)
	{
	    ...
	    if (in_array($data['call_type'], array(1, 2)) && isset($data['sip_profile']))
	    {
	        $data['sip_profile_id'] = (int) $data['sip_profile'];
	    }
	    else
	    {
	        $data['sip_profile_id'] = 0;
	    }
	
	    unset($data['action']);
	    unset($data['sip_profile']);
	    unset($data['id']);
	...
	function did_batch_update($update_array)
	{
		unset($update_array['action']);
		$date = gmdate("Y-m-d h:i:s");
		$this->db_model->build_search('did_list_search');
		$reseller_info = $this->session->userdata['accountinfo'];
		if ($reseller_info['type'] == 1 || $reseller_info['type'] == 5)
		{
			$this->db->where('parent_id', $reseller_info['id']);
		}
		$this->db_model->build_search('did_list_search');

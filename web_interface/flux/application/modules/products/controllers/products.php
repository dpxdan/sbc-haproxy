...
class Products extends MX_Controller {
	...
	function products_add($category="") {
		...
		if(isset($_POST['product_category']) && $_POST['product_category'] != ''){
			...
			if ($category == "DID"){
				$data ['bypass_media']=Common_model::$global_config['system_config']['enable_bypass_media'];
			}
		...
		if ($add_array->num_rows > 0) {
			...
			if($product_info['product_category'] == 4){
				...
				if($did_info->num_rows > 0){
					...
					$did_info['is_recording'] = $this->common->get_field_name("is_recording","accounts",array("id"=>$did_info['accountid']));
					} 
					else {
					$did_info['is_recording'] = 0;
					}
			...
			$category = $this->common->get_field_name("name","category",array("id"=>$product_info['product_category']));
			  ...
			  if ($this->form_validation->run() == FALSE){ 	
				...
				if(isset($add_array) && !empty($add_array)){
					...
					$assign_summary = $this->$Method($add_array);
					if($add_array['product_category'] == "DID"){
						...
						$update_message = gettext('Product updated successfully!');
						if (!empty($assign_summary)) {
							$update_message .= ' ' . $assign_summary;
						}
						$this->session->set_flashdata ( 'flux_errormsg', $update_message);
						redirect ( base_url () . 'products/products_list/' );
					}
		  }
		}
	...
	function products_check_rategroup_conflicts() {
		$this->output->set_content_type('application/json');
		$accountinfo = $this->session->userdata('accountinfo');
		$reseller_id = $accountinfo['type'] == 1 ? $accountinfo['id'] : 0;

		$product_id      = (int) $this->input->post('product_id');
		$rate_groups_raw = $this->input->post('product_rate_group');

		if (empty($rate_groups_raw)) {
			echo json_encode(array('has_conflicts' => false, 'conflicts' => array()));
			return;
		}

		$rate_groups = is_array($rate_groups_raw) ? $rate_groups_raw : explode(',', $rate_groups_raw);
		$rate_groups = array_values(array_filter(array_map('intval', $rate_groups)));
		...
		$this->db->distinct();
		$this->db->select('a.id as account_id, a.number, a.first_name, a.last_name, oi.product_id as existing_product_id, p.name as existing_product_name');
		$this->db->from('accounts a');
		$this->db->join('order_items oi', 'oi.accountid = a.id AND oi.is_terminated = 0 AND oi.product_category = 1', 'inner');
		$this->db->join('products p', 'p.id = oi.product_id', 'inner');
		$this->db->where(array('a.status' => 0, 'a.deleted' => 0, 'a.type' => 0, 'a.reseller_id' => $reseller_id));
		$this->db->where_in('a.pricelist_id', $rate_groups);
		if ($product_id) {
		$this->db->where('oi.product_id !=', $product_id);
		$this->db->where('a.id NOT IN (SELECT accountid FROM order_items WHERE product_id = '.$product_id.' AND is_terminated = 0)', NULL, FALSE);
		}
		$result = $this->db->get();

		$conflicts = ($result->num_rows > 0) ? $result->result_array() : array();
	...
	function assign_product_to_exiting_account($productinfo, $product_id, $conflict_action = '') {
		$productinfo['product_id'] = $product_id;
		$accountinfo = $this->session->userdata('accountinfo');
		$reseller_id = $accountinfo['type'] == 1 ? $accountinfo['id'] : 0;

		$has_rate_group = isset($productinfo['product_rate_group']) && !empty($productinfo['product_rate_group']);

		if ($productinfo['apply_on_existing_account'] != 0 || !$has_rate_group) {
			return false;
		}

		$rate_groups = is_array($productinfo['product_rate_group'])
			? $productinfo['product_rate_group']
			: explode(',', $productinfo['product_rate_group']);

		$this->db->select('*');
		$this->db->from('accounts');
		$this->db->where(array('status' => 0, 'deleted' => 0, 'type' => 0, 'reseller_id' => $reseller_id));
		$this->db->where_in('pricelist_id', $rate_groups);
		...
		$summary = array('created' => 0, 'skipped' => 0, 'no_balance' => 0, 'replaced' => 0, 'errors' => 0);

		foreach ($account_info as $account) {
			$already_has = $this->db->get_where('order_items', array(
				'accountid'     => $account['id'],
				'product_id'    => $product_id,
				'is_terminated' => 0,
			));
			if ($already_has->num_rows > 0) {
				$summary['skipped']++;
				continue;
			}

			if ($productinfo['release_no_balance'] == 0) {
				$account_balance = $account['posttoexternal'] == 1
			...
			try {
				if ($conflict_action === 'replace') {
					...
					$old_items = $this->db->get('order_items');

					if ($old_items->num_rows > 0) {
						foreach ($old_items->result_array() as $old_item) {
							$this->db->where(array(
								'accountid'     => $account['id'],
							...
							$this->db->where(array('accountid' => $account['id'], 'product_id' => $old_item['product_id']));
			...
			$customer_data = ($customer_data->num_rows > 0) ? $customer_data->result_array()[0] : array();

			if (!empty($customer_data) && isset($productinfo['email_notify']) && $productinfo['email_notify'] == 1 && $last_id > 0) {
				$cat_label = ($productinfo['product_category'] == 1) ? 'PACKAGE' : (($productinfo['product_category'] == 2) ? 'SUBSCRIPTION' : 'DID');
				$productinfo['next_billing_date'] = ($productinfo['billing_days'] == 0)
					? gmdate('Y-m-d 23:59:59', strtotime('+10 years'))
					: gmdate('Y-m-d 23:59:59', strtotime('+'.($productinfo['billing_days'] - 1).' days'));
				$final_array = array_merge($customer_data, $productinfo);
				$final_array['quantity']           = ($cat_label === 'SUBSCRIPTION') ? (isset($productinfo['quantity']) ? $productinfo['quantity'] : 1) : 1;
				$final_array['category_name']      = $cat_label;
				$final_array['price']              = $productinfo['setup_fee'] + $productinfo['price'];
				$final_array['total_price']        = $final_array['price'] * $final_array['quantity'];
				$final_array['total_price_amount'] = $final_array['price'];
				if ($productinfo['release_no_balance'] == 0) {
	...
	function format_assign_summary($summary) {
		return sprintf(
			gettext('Applied to existing accounts: %d created, %d replaced, %d skipped (already had it), %d without balance, %d errors.'),
			$summary['created'], $summary['replaced'], $summary['skipped'], $summary['no_balance'], $summary['errors']
		);
	}

	function package_product($add_array) {
		$SearchArr    = '';
		$conflict_action = isset($add_array['conflict_action']) ? $add_array['conflict_action'] : '';

		if (isset($add_array['id']) && $add_array['id'] != '') {
			$this->product_model->edit_product($add_array, $add_array['id'], $SearchArr);
			if ($add_array['apply_on_existing_account'] == 0) {
				$summary = $this->assign_product_to_exiting_account($add_array, $add_array['id'], $conflict_action);
				if (is_array($summary)) {
					return $this->format_assign_summary($summary);
				}
			}
			...
			$message = gettext('Package created successfully!');

			if ($add_array['apply_on_existing_account'] == 0) {
				$summary = $this->assign_product_to_exiting_account($add_array, $last_id, $conflict_action);
				if (is_array($summary)) {
					$message .= ' ' . $this->format_assign_summary($summary);
				}
			}

			$this->session->set_flashdata('flux_errormsg', $message);
	...
	function products_did(){  
		$data ['page_title'] = gettext ( 'Create Product' );
		$accountinfo = $this->session->userdata ( "accountinfo" );
		$data['product_category'] = $this->db_model->build_dropdown_products("id,name,code", "category", "name", "DID");
		$data ['bypass_media']=Common_model::$global_config['system_config']['enable_bypass_media'];
		$data['currency'] = $this->common->get_field_name("currency","currency",array("id"=>$accountinfo['currency_id']));
		$reseller_id = $accountinfo ['type'] == 1 ? $accountinfo ['id'] : 0;

...
function get_alert_msg(id) {
    ...
    confirm_string = gettext_custom('Are you sure want to delete?');
...
function get_alert_msg_restore(id) {
    confirm_string = gettext_custom('Are you sure want to restore this database?');
    var answer = confirm(confirm_string);
    return answer; // answer is a boolean
}
    ...
    for (var i = 0; i < col_field.length; i++) {
        if (col_field[i] != "") {
            ...
            if (col_str != 'null' && col_str != '') {
                ...
                var custom_display = collumn_property[0];
                jsonObj.push({
...
function build_buttons(buttons_arr) {
    ...
    for (var i = 0; i < btn_field.length; i++) {
        if (btn_field[i] != "") {
            ...
            var permission_cached = null;
            if (typeof button_permission_cache !== 'undefined' &&
                Object.prototype.hasOwnProperty.call(button_permission_cache, button_property[7])) {
                permission_cached = button_permission_cache[button_property[7]];
            } else if (typeof button_permission_allow_all !== 'undefined' && button_permission_allow_all) {
                permission_cached = '0';
            }

            if (permission_cached !== null) {
                build_buttons_push(jsonObj, permission_cached, button_property, newURL, layout);
                continue;
            }
            ...
            $.ajax({
                ...
                data: post_data,
                success: function (response) {
                    build_buttons_push(jsonObj, response.trim(), button_property, newURL, layout);
                }
            });
        }
    }
...
function build_buttons_push(jsonObj, permission, button_property, newURL, layout) {
    if (permission != 0) {
        return;
    }
...
var gettext_custom_cache = gettext_custom_cache || {};

function gettext_custom(collumn_property) {

    var cache_key = String(collumn_property);
    if (Object.prototype.hasOwnProperty.call(gettext_custom_cache, cache_key)) {
        return gettext_custom_cache[cache_key];
    }
    ...
    if (collumn !== '') {
        gettext_custom_cache[cache_key] = collumn;
    }
    return collumn;
}

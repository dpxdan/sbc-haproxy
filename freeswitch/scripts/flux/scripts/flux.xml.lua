...
function freeswitch_xml_header(xml,destination_number,accountcode,maxlength,call_direction,accountname,xml_user_rates,customer_userinfo,config,xml_did_rates,reseller_cc_limit,callerid_array,original_destination_number,dialed_destination_number)
	...
	local sched_maxlength = tonumber(maxlength) or 0
	local config_max_length = tonumber(config['call_max_length'])
	if sched_maxlength <= 0 or (config_max_length and sched_maxlength > config_max_length) then
		sched_maxlength = config_max_length or 100
		Logger.notice("[XML_HEADER] Limiting call to config max length "..sched_maxlength.." mins! (maxlength was "..tostring(maxlength)..")");
	end
	Logger.debug("maxlength::::::::: "..sched_maxlength);
	table.insert(xml, [[<action application="set" data="bridge_pre_execute_bleg_app=sched_hangup"/>]]);
	table.insert(xml, [[<action application="set" data="bridge_pre_execute_bleg_data=+]]..math.floor(sched_maxlength * 60)..[[ normal_clearing"/>]]);
	...
	-- sched_hangup no A-leg como rede de segurança: garante que o canal seja destruído
	-- mesmo que o bridge nunca conecte. Margem de +60s sobre o B-leg para evitar race condition.
	local a_leg_timeout = math.floor(sched_maxlength * 60) + 60
	table.insert(xml, [[<action application="sched_hangup" data="+]]..a_leg_timeout..[[ normal_clearing"/>]]);
	...
	if(tonumber(config['minutes_announce']) == 0) then
		...
		table.insert(xml, [[<action application="say" data="en NUMBER PRONOUNCED ]].. math.floor(sched_maxlength)..[["/>]]);
	...
	elseif (tonumber(customer_userinfo['is_recording']) == 1 and tonumber(customer_userinfo['bypass_media']) == 0) then
		table.insert(xml, [[<action application="export" data="bypass_media=true"/>]]);
	else
		table.insert(xml, [[<action application="export" data="bypass_media=false"/>]]);
	end
	return xml
end
...
function custom_inbound_1(xml, didinfo, userinfo, config, xml_did_rates, callerid_array, livecall_data)
	...
	local did_local_chan = ""
	local sip_profile_name = didinfo['sip_profile_name'];
	if (sip_profile_name and sip_profile_name ~= "") then
		table.insert(xml, [[<action application="export" data="sofia_profile_name=]] .. sip_profile_name .. [["/>]])
	end
	if (params:getHeader('variable_sip_from_user') and params:getHeader('variable_sip_from_user') ~= "") then
		local sip_codec = get_sip_codec(params:getHeader('variable_sip_from_user'))
		if (sip_codec and sip_codec ~= "") then
	...
	table.insert(xml,
		...
		didinfo['leg_timeout'] .. did_local_chan .. [[]sofia/${sofia_profile_name}/]] .. didinfo['extensions'] .. [["/>]]);
	return xml;
end
...
function custom_inbound_2(xml, didinfo, userinfo, config, xml_did_rates, callerid_array, livecall_data)
	...
	table.insert(xml,
		[[<action application="bridge" data="[leg_timeout=]] ..
		didinfo['leg_timeout'] ..
		did_local_chan .. [[]sofia/${sofia_profile_name}/]] .. destination_number ..
		[[@]] .. didinfo['extensions'] .. [["/>]]);
	return xml;
end
